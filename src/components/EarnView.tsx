  ...
  return (
    ...
    >
      ...
      <div className="rounded-2xl bg-zinc-950 border border-zinc-900 p-5 mb-6 relative overflow-hidden">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xs font-semibold tracking-wide uppercase text-zinc-300">Active Attention Yield</span>
          </div>
      ...
      {cpagrip && (
        <div className="rounded-2xl bg-zinc-950 border border-zinc-900 p-5 mb-6">
          <div className="flex items-center justify-between mb-2">
            <div>
              <h3 className="text-sm font-bold text-white">CPAGrip Offers</h3>
              <p className="text-[11px] text-zinc-400 mt-1">Offerwall and Smartlink use your Sphere ID as CPAGrip tracking_id.</p>
            </div>
            <span className={`text-[10px] px-2 py-0.5 rounded border ${cpagrip.isConfigured ? 'text-emerald-400 border-emerald-800 bg-emerald-950' : 'text-zinc-500 border-zinc-800 bg-zinc-900'}`}>
              {cpagrip.isConfigured ? 'READY' : 'PENDING'}
            </span>
          </div>
          ...
          <button type="button" onClick={handleStartCpagrip} disabled={openingCpagrip} className="w-full mt-3 py-2.5 rounded-xl bg-white text-black font-semibold text-xs disabled:opacity-50 flex items-center justify-center space-x-2">
            {openingCpagrip ? <Loader2 className="w-4 h-4 animate-spin" /> : <ExternalLink className="w-4 h-4" />}
            <span>Open CPAGrip Smartlink</span>
          </button>
      ...
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Reward Provider Abstraction</h3>
          ...
          {providers.map((provider) => (
            <div key={provider.id} className="p-4 rounded-xl bg-zinc-950 border border-zinc-900 text-xs">
              <div className="flex items-center justify-between mb-1.5">
                ...
                <span className={`inline-flex items-center space-x-1 px-2 py-0.5 rounded text-[10px] font-mono ${provider.isConfigured ? 'bg-emerald-950 text-emerald-400 border border-emerald-800' : 'bg-zinc-900 text-zinc-500 border border-zinc-800'}`}>
                  {provider.isConfigured ? <><CheckCircle2 className="w-3 h-3 text-emerald-400" /><span>CONNECTED</span></> : <><Clock className="w-3 h-3 text-zinc-500" /><span>PENDING CREDENTIALS</span></>}
                </span>
              </div>
              <p className="text-[11px] text-zinc-400 mb-2.5 leading-relaxed">{provider.description}</p>
              <div className="flex flex-wrap gap-1.5 mb-2.5">
                {provider.supportedOfferTypes.map((type) => <span key={type} className="px-2 py-0.5 rounded bg-zinc-900/90 text-zinc-400 text-[10px] border border-zinc-800/80">{type}</span>)}
              </div>
              ...
              <div className="border-t border-zinc-900 pt-2.5">
                <button type="button" onClick={() => setExpandedId(expandedId === provider.id ? null : provider.id)} className="w-full flex items-center justify-between text-[11px] text-zinc-400 hover:text-white py-1 transition-colors">
                  <span className="font-semibold">Publisher Postback & Sandbox Tools</span>
                  {expandedId === provider.id ? <ChevronUp className="w-3.5 h-3.5 text-zinc-400" /> : <ChevronDown className="w-3.5 h-3.5 text-zinc-400" />}
                </button>
                  ...
                  <div className="mt-2.5 p-3 rounded-lg bg-black border border-zinc-800 space-y-3">
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <button type="button" onClick={() => handleCopyPostback(provider.id)} className="text-[10px] text-zinc-400 hover:text-white flex items-center space-x-1">
                          {copiedId === provider.id ? <><Check className="w-3 h-3 text-emerald-400" /><span className="text-emerald-400">Copied</span></> : <><Copy className="w-3 h-3" /><span>Copy URL</span></>}
                        </button>
                      </div>
                      <div className="p-2 rounded bg-zinc-900 border border-zinc-800 text-[10px] font-mono text-zinc-300 break-all select-all">
                        {provider.id === 'cpalead' ? `${window.location.origin}/api/cpal_postback?subid={subid}&lead_id={lead_id}&campaign_id={campaign_id}&campaign_name={campaign_name}&payout={payout}&password={password}` : provider.id === 'cpagrip' ? `${window.location.origin}/api/earn/postback/cpagrip` : `${window.location.origin}/api/earn/postback/${provider.id}`}
                      </div>
                      <p className="text-[10px] text-zinc-500 mt-1">Use the provider-specific contract above. Secrets remain server-side.</p>
                    </div>
                    <div className="flex items-center justify-between pt-1">
                      <span className="text-[10px] text-zinc-400">Verify persistent wallet pipeline:</span>
                      <button type="button" onClick={() => handleTestReward(provider.id)} disabled={testingId === provider.id} className="px-2.5 py-1.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-white text-[10px] font-semibold flex items-center space-x-1.5 transition-colors disabled:opacity-40">
                        {testingId === provider.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <Play className="w-3 h-3 text-emerald-400" />}<span>Test Reward Credit (+$0.25)</span>
                      </button>
                    </div>
                  </div>
                )}
