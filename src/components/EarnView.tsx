        ...
        <div className="rounded-2xl bg-zinc-950 border border-zinc-900 p-5 mb-6">
          <div className="flex items-center justify-between mb-2">
            <div>
              ...
              <p className="text-[11px] text-zinc-400 mt-1">Your Sphere user ID is attached as CPAGrip tracking_id.</p>
            </div>
          ...
          <button
            type="button"
            onClick={handleStartCpagrip}
            disabled={openingCpagrip}
            className="w-full mt-3 py-2.5 rounded-xl bg-white text-black font-semibold text-xs disabled:opacity-50 flex items-center justify-center space-x-2"
          >
            {openingCpagrip ? <Loader2 className="w-4 h-4 animate-spin" /> : <ExternalLink className="w-4 h-4" />}
            <span>Start CPAGrip Offers</span>
          </button>
                  ...
                  <div className="mt-2.5 p-3 rounded-lg bg-black border border-zinc-850 space-y-3">
                    <div>
                      ...
                      <div className="p-2 rounded bg-zinc-900 border border-zinc-800 text-[10px] font-mono text-zinc-300 break-all select-all">
                        {provider.id === 'cpalead'
                          ? `${window.location.origin}/api/cpal_postback?subid={subid}&lead_id={lead_id}&campaign_id={campaign_id}&campaign_name={campaign_name}&payout={payout}&password={password}`
                          : provider.id === 'cpagrip'
                            ? `${window.location.origin}/api/earn/postback/cpagrip`
                            : `${window.location.origin}/api/earn/postback/${provider.id}`}
                      </div>
                      <p className="text-[10px] text-zinc-500 mt-1">
                        Use the provider-specific contract above. Secrets remain server-side.
                      </p>
                    </div>
