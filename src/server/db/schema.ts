...
export async function initializeDatabase(db: DatabaseAdapter): Promise<void> {
  ...
  try {
    ...
    try { await db.exec("CREATE UNIQUE INDEX IF NOT EXISTS idx_withdrawals_pending_user_unique ON withdrawals(user_id) WHERE status = 'pending'"); } catch (error: any) { console.warn('[Sphere DB] Pending-withdrawal uniqueness index deferred:', error?.message || error); }
