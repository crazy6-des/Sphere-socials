...
class FakeDb {
  ...
  wallets: Row[] = [{ id: 'wallet_test', user_id: 'CPA_TEST_USER_001', balance: 0, total_earned: 0 }];
  events: Row[] = [];
  transactions: Row[] = [];
  ...
  async run(sql: string, p: any[]) {
    ...
    if (sql.startsWith('INSERT INTO transactions')) {
      ...
      const isReversal = String(p[5]).toLowerCase().includes('reversal');
      if (!isReversal && event.status !== 'pending') return { success: true, meta: { changes: 0 } };
      if (isReversal && event.status !== 'credited') return { success: true, meta: { changes: 0 } };
      this.transactions.push({ id: p[0], wallet_id: p[1], user_id: p[2], amount: p[3], provider: p[4], description: p[5], reference_id: ref, status: isReversal ? 'cancelled' : 'completed' });
      return { success: true, meta: { changes: 1 } };
    }
    ...
    if (sql.startsWith('UPDATE wallets SET balance = balance -')) {
      ...
      if (!w || w.balance < p[0]) return { success: true, meta: { changes: 0 } };
...
async function call(db: FakeDb, payload: Record<string, string>, env: Record<string, string>) {
  ...
  return handlePr2HardenedRequest(new Request(`https://isolated.test/api/earn/postback/cpalead?${query}`, { method: 'GET' }), env, db as any);
}
...
async function main() {
  ...
  const db = new FakeDb();
  const valid = { subid: 'CPA_TEST_USER_001', lead_id: 'TEST_LEAD_001', campaign_id: 'CAMP_001', campaign_name: 'Test Campaign', payout: '1.00', password: 'test-secret' };
  ...
  const missingId = await call(db, { ...valid, lead_id: '' }, env);
  assert.ok(missingId.status >= 400 && missingId.status < 500);
  assert.equal(db.wallets[0].balance, 0);

  const unknownUser = await call(db, { ...valid, subid: 'UNKNOWN_USER' }, env);
  ...
  assert.equal(reversalJson.success, true);
  assert.equal(db.wallets[0].balance, 0);
  assert.equal(db.events[0].status, 'reversed');
  assert.equal(db.transactions.length, 2);

  const reversalDuplicate = await call(db, { ...valid, status: 'reversed' }, env);
  assert.ok(reversalDuplicate.status >= 200 && reversalDuplicate.status < 300);
  assert.equal(db.wallets[0].balance, 0);
  assert.equal(db.transactions.length, 2);

  console.log('CPAlead isolated handler tests passed: bad secret, missing ID, unknown user, credit, 75/25 split, duplicate retry, reversal, reversal retry');
}
