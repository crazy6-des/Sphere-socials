...
async function call(db:FakeDb, payload:Record<string,string>, env:Record<string,string>){
 const query=new URLSearchParams(payload);return handlePr2HardenedRequest(new Request(`https://isolated.test/api/cpal_postback?${query}`,{method:'GET'}),env,db as any);
}
async function body(response:Response){return await response.json() as any}

async function main(){
 const env={CPALEAD_POSTBACK_PASSWORD:'test-secret',REWARD_USER_SHARE_PERCENT:'75'};const db=new FakeDb();
 ...
 const duplicate=await call(db,valid,env);assert.ok(duplicate.status>=200&&duplicate.status<300);assert.equal(db.wallets[0].balance,0.75);assert.equal(db.transactions.length,1);
 console.log('CPAlead isolated handler tests passed: bad secret, missing ID, unknown user, live-shaped credit, 75/25 split, duplicate retry');
}
