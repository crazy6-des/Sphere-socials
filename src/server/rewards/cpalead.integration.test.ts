...
async function call(db:FakeDb,payload:Record<string,string>,env:Record<string,string>){const query=new URLSearchParams(payload);return handlePr2HardenedRequest(new Request(`https://isolated.test/api/earn/postback/cpalead?${query}`,{method:'GET'}),env,db as any)}
...
async function main(){
 ...
 console.log('CPAlead isolated handler tests passed: bad secret, missing ID, unknown user, credit, 75/25 split, duplicate retry, reversal, safe reversal retry');
}
