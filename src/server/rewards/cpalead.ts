...
function json(request: Request, body: unknown, status = 200, extra: Record<string, string> = {}): Response {
  return new Response(JSON.stringify(body), {
    status,
    headers: { 'Content-Type': 'application/json', 'Cache-Control': 'no-store', ...cors(request), ...extra },
  });
}
...
async function offers(request: Request, env: CpaleadEnv, db: any, url: URL): Promise<Response> {
  ...
  if (!userId) return json(request, { success: false, error: 'Authentication required.' }, 401);

  const publisherId = envValue(env, 'CPALEAD_PUBLISHER_ID');
  if (!publisherId) return json(request, { success: false, error: 'CPAlead publisher configuration is unavailable.' }, 503);
  ...
  try {
    response = await fetch(upstream.toString(), { method: 'GET', headers: { Accept: 'application/json' }, cache: 'no-store' });
  } catch {
    return json(request, { success: false, error: 'CPAlead Offers API is unreachable.' }, 502);
  }

  const payload: any = await response.json().catch(() => null);
  if (!response.ok || !payload) {
    return json(request, { success: false, error: 'CPAlead Offers API returned an invalid response.' }, 502);
  }

  const rawOffers = Array.isArray(payload.offers) ? payload.offers : Array.isArray(payload.data) ? payload.data : [];
  const normalized = rawOffers.map((offer: any) => ({
    id: clean(offer.id || offer.offer_id, 100),
    title: clean(offer.title, 200),
    description: clean(offer.description, 1000),
    longDescription: clean(offer.long_description, 4000),
    link: clean(offer.link, 2000),
    amount: Number(offer.amount || 0),
    currency: clean(offer.payout_currency || 'USD', 16).toUpperCase(),
    offerRank: Number(offer.offer_rank || 0),
    device: offer.device || null,
    conversionMode: offer.conversion_mode || null,
    countries: offer.countries || [],
    events: Array.isArray(offer.events) ? offer.events : [],
    creatives: offer.creatives || null,
    previewLink: clean(offer.preview_link, 2000),
  })).filter((offer: any) => offer.id && offer.link);
...
export async function handleCpaleadRequest(request: Request, env: CpaleadEnv, db: any): Promise<Response | null> {
  ...
  if (request.method.toUpperCase() === 'OPTIONS') return json(request, { success: true }, 204);
