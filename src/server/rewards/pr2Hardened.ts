...
function cors(request: Request): Record<string, string> { const origin = request.headers.get('Origin') || ''; const allowed = !origin || origin === 'null' || /^https:\/\/([a-z0-9-]+\.)*netlify\.app$/i.test(origin) || /^http:\/\/(localhost|127\.0\.0\.1)(:\d+)?$/i.test(origin); return { 'Access-Control-Allow-Origin': allowed && origin && origin !== 'null' ? origin : '*', 'Access-Control-Allow-Credentials': 'true', Vary: 'Origin' }; }
function json(request: Request, body: unknown, status = 200): Response { return new Response(JSON.stringify(body), { status, headers: { 'Content-Type': 'application/json', ...cors(request) } }); }
...
async function rewardCredit(db: DatabaseAdapter, conversion: Conversion, env: any): Promise<{ state: 'credited' | 'duplicate' | 'reversed' | 'rejected'; balance?: number; grossPayout?: number; userReward?: number; platformShare?: number }> {
  ...
  if (conversion.status === 'approved') {
    ...
    const batch = await db.batch([
      ...
      db.prepare("UPDATE wallets SET balance = balance + ?, total_earned = total_earned + ?, updated_at = ? WHERE user_id = ? AND EXISTS (SELECT 1 FROM transactions WHERE reference_id = ? AND status = 'completed') AND EXISTS (SELECT 1 FROM reward_events WHERE provider = ? AND external_conversion_id = ? AND status = 'pending')").bind(userReward, userReward, Date.now(), user.id, reference, conversion.provider, conversion.externalConversionId),
      db.prepare("UPDATE reward_events SET status = 'credited', transaction_id = COALESCE((SELECT id FROM transactions WHERE reference_id = ? ORDER BY created_at DESC LIMIT 1), transaction_id), processed_at = ? WHERE provider = ? AND external_conversion_id = ? AND status = 'pending' AND EXISTS (SELECT 1 FROM transactions WHERE reference_id = ? AND status = 'completed')").bind(reference, Date.now(), conversion.provider, conversion.externalConversionId, reference),
    ]);
  ...
  if (!batch?.[0]?.meta?.changes) return { state: 'duplicate' }; const updated: any = await db.prepare('SELECT balance FROM wallets WHERE user_id = ?').bind(user.id).first(); return { state: 'reversed', balance: Number(updated?.balance || 0), grossPayout: Number(event.payout || conversion.payout), userReward: amount, platformShare: Math.round((Number(event.payout || conversion.payout) - amount) * 100) / 100 };
}

async function walletResponse(request: Request, db: DatabaseAdapter, userId: string, env: any): Promise<Response> { const wallet: any = await db.prepare('SELECT user_id, balance, total_earned, total_withdrawn, updated_at FROM wallets WHERE user_id = ?').bind(userId).first(); const transactions: any = await db.prepare('SELECT id, wallet_id, user_id, type, amount, status, provider, description, reference_id, created_at FROM transactions WHERE user_id = ? ORDER BY created_at DESC LIMIT 100').bind(userId).all(); const w = wallet || { user_id: userId, balance: 0, total_earned: 0, total_withdrawn: 0, updated_at: Date.now() }; const share = rewardSharePercent(env); return json(request, { success: true, data: { wallet: { userId: w.user_id, balance: Number(w.balance || 0), totalEarned: Number(w.total_earned || 0), totalWithdrawn: Number(w.total_withdrawn || 0), updatedAt: Number(w.updated_at || Date.now()) }, rewardPolicy: { userSharePercent: share, platformSharePercent: Math.round((100 - share) * 100) / 100 }, transactions: transactions.results || [] } }); }
...
export async function handlePr2HardenedRequest(request: Request, env: any, db: DatabaseAdapter): Promise<Response | null> {
  ...
  if (path === '/api/earn/attention-reward' && method === 'POST') { const wallet: any = await db.prepare('SELECT id FROM wallets WHERE user_id = ?').bind(userId).first(); if (!wallet) return fail(request, 'Wallet unavailable.', 500); const day = new Date(); day.setHours(0, 0, 0, 0); const reference = `attention:${userId}:${day.getTime()}`; if (!db.batch) return fail(request, 'Reward service unavailable.', 503); const pending: any = await db.prepare('SELECT id, status FROM transactions WHERE reference_id = ?').bind(reference).first(); if (pending?.status === 'completed') return fail(request, 'Daily attention reward already credited.', 409); if (!pending) { const inserted = await db.prepare('INSERT OR IGNORE INTO transactions (id, wallet_id, user_id, type, amount, status, provider, description, reference_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').bind(`txn_${generateId(14)}`, wallet.id, userId, 'reward', 0.15, 'pending', 'attention', 'Verified attention session', reference, Date.now()).run(); if (!inserted.success) return fail(request, 'Reward service unavailable.', 503); } const batch = await db.batch([db.prepare("UPDATE wallets SET balance = balance + 0.15, total_earned = total_earned + 0.15, updated_at = ? WHERE user_id = ? AND EXISTS (SELECT 1 FROM transactions WHERE reference_id = ? AND status = 'pending')").bind(Date.now(), userId, reference), db.prepare("UPDATE transactions SET status = 'completed' WHERE reference_id = ? AND status = 'pending'").bind(reference)]); if (!batch?.[0]?.meta?.changes) return fail(request, 'Daily attention reward already credited.', 409); const updated: any = await db.prepare('SELECT balance FROM wallets WHERE user_id = ?').bind(userId).first(); return json(request, { success: true, data: { rewardAmount: 0.15, newBalance: Number(updated?.balance || 0), message: 'Attention session verified and credited.' } }); }
  if (path === '/api/earn/simulate-reward' && method === 'POST') { if (envValue(env, 'NODE_ENV') === 'production' || envValue(env, 'PRODUCTION') === 'true') return fail(request, 'Reward simulation is disabled in production.', 403); const body: any = await request.json().catch(() => ({})); const provider = clean(body.providerId || body.provider_id).toLowerCase(); const amount = money(body.amount); if (!['cpalead', 'cpagrip'].includes(provider) || amount === null) return fail(request, 'Invalid provider or reward amount.'); const wallet: any = await db.prepare('SELECT id FROM wallets WHERE user_id = ?').bind(userId).first(); if (!wallet || !db.batch) return fail(request, 'Wallet unavailable.', 500); const now = Date.now(); const simulationReference = `simulation:${provider}:${generateId(16)}`; await db.batch([db.prepare('INSERT INTO transactions (id, wallet_id, user_id, type, amount, status, provider, description, reference_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').bind(`txn_${generateId(14)}`, wallet.id, userId, 'reward', amount, 'completed', provider, 'Development reward simulation', simulationReference, now), db.prepare('UPDATE wallets SET balance = balance + ?, total_earned = total_earned + ?, updated_at = ? WHERE user_id = ?').bind(amount, amount, now, userId)]); const updated: any = await db.prepare('SELECT balance FROM wallets WHERE user_id = ?').bind(userId).first(); return json(request, { success: true, data: { rewardAmount: amount, newBalance: Number(updated?.balance || 0), message: 'Development reward simulated.' } }); }
  return null;
}
