...
async function rewardCredit(db: DatabaseAdapter, conversion: Conversion): Promise<{ state: 'credited' | 'duplicate' | 'reversed' | 'rejected'; balance?: number }> {
  ...
  const wallet: any = await db.prepare('SELECT id FROM wallets WHERE user_id = ?').bind(user.id).first(); if (!wallet || !db.batch) return { state: 'rejected' };
  if (conversion.status === 'approved') {
    ...
    const txId = `txn_${generateId(14)}`; const reference = `reward:${conversion.provider}:${conversion.externalConversionId}`;
    ...
    if (!batch?.[0]?.meta?.changes) return { state: 'duplicate' }; const updated: any = await db.prepare('SELECT balance FROM wallets WHERE user_id = ?').bind(user.id).first(); return { state: 'credited', balance: Number(updated?.balance || 0) };
  }
  ...
  const reversalReference = `reward-reversal:${conversion.provider}:${conversion.externalConversionId}`; const amount = Number(event.payout || conversion.payout); if (!Number.isFinite(amount) || amount <= 0) return { state: 'rejected' };
  const batch = await db.batch([
  ...
  if (!batch?.[0]?.meta?.changes) return { state: 'duplicate' }; const updated: any = await db.prepare('SELECT balance FROM wallets WHERE user_id = ?').bind(user.id).first(); return { state: 'reversed', balance: Number(updated?.balance || 0) };
}

async function walletResponse(request: Request, db: DatabaseAdapter, userId: string): Promise<Response> { const wallet: any = await db.prepare('SELECT user_id, balance, total_earned, total_withdrawn, updated_at FROM wallets WHERE user_id = ?').bind(userId).first(); const transactions: any = await db.prepare('SELECT id, wallet_id, user_id, type, amount, status, provider, description, reference_id, created_at FROM transactions WHERE user_id = ? ORDER BY created_at DESC LIMIT 100').bind(userId).all(); const w = wallet || { user_id: userId, balance: 0, total_earned: 0, total_withdrawn: 0, updated_at: Date.now() }; return json(request, { success: true, data: { wallet: { userId: w.user_id, balance: Number(w.balance || 0), totalEarned: Number(w.total_earned || 0), totalWithdrawn: Number(w.total_withdrawn || 0), updatedAt: Number(w.updated_at || Date.now()) }, transactions: transactions.results || [] } }); }
