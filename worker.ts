...
function workerErrorResponse(request: Request, error: string, status: number): Response { return new Response(JSON.stringify({ success: false, error }), { status, headers: { 'Content-Type': 'application/json', ...workerCorsHeaders(request) } }); }

async function guardLoginRequest(request: Request, env: Env): Promise<Response | null> {
  if (request.method.toUpperCase() !== 'POST') return null;
  const url = new URL(request.url); let path = url.pathname; if (path.startsWith('/.netlify/functions/api')) path = path.replace('/.netlify/functions/api', '/api'); if (path !== '/api/auth/login') return null;
  const body: any = await request.clone().json().catch(() => ({})); const identifier = typeof body?.identifier === 'string' ? body.identifier.toLowerCase().trim() : ''; const password = typeof body?.password === 'string' ? body.password : ''; if (!identifier || !password) return null;
  ...
  const row = await env.DB.prepare('SELECT id FROM users WHERE username = ? OR email = ?').bind(identifier, identifier).first(); if (!row) return workerErrorResponse(request, 'Invalid username/email or password.', 401); return null;
}

async function guardSessionBoundary(request: Request, env: Env, secret: string): Promise<Response | null> {
  const authHeader = request.headers.get('Authorization') || request.headers.get('authorization'); if (!authHeader?.startsWith('Bearer ')) return null;
  const url = new URL(request.url); let path = url.pathname; if (path.startsWith('/.netlify/functions/api')) path = path.replace('/.netlify/functions/api', '/api'); if (path.startsWith('/api/auth/')) return null;
  const actorId = await authenticateSocialRequest(request, env.DB, secret).catch(() => null); if (actorId) return null; return workerErrorResponse(request, 'Invalid, expired, or revoked authentication session.', 401);
}
...
async function normalizePostMedia(response: Response, request: Request): Promise<Response> {
  ...
  const body: any = await response.clone().json().catch(() => null); if (!body?.data) return response; let changed = false;
  ...
  if (!changed) return response; const headers = new Headers(response.headers); headers.delete('content-length'); return new Response(JSON.stringify(body), { status: response.status, headers });
}

async function captureNotificationContext(request: Request, env: Env, actorId: string | null): Promise<any> {
  if (!actorId || !env?.DB) return null; const url = new URL(request.url); let path = url.pathname; if (path.startsWith('/.netlify/functions/api')) path = path.replace('/.netlify/functions/api', '/api');
  const likeMatch = path.match(/^\/api\/posts\/([a-zA-Z0-9_-]+)\/like$/); if (request.method.toUpperCase() === 'POST' && likeMatch) { const post: any = await env.DB.prepare('SELECT user_id FROM posts WHERE id = ?').bind(likeMatch[1]).first(); const existing: any = await env.DB.prepare('SELECT id FROM likes WHERE user_id = ? AND post_id = ?').bind(actorId, likeMatch[1]).first(); return { type: 'like', postId: likeMatch[1], targetUserId: post?.user_id || null, wasExisting: Boolean(existing) }; }
  const followMatch = path.match(/^\/api\/users\/([a-zA-Z0-9_-]+)\/follow$/); if (request.method.toUpperCase() === 'POST' && followMatch) { const existing: any = await env.DB.prepare('SELECT id FROM follows WHERE follower_id = ? AND following_id = ?').bind(actorId, followMatch[1]).first(); return { type: 'follow', targetUserId: followMatch[1], wasExisting: Boolean(existing) }; }
  const commentMatch = path.match(/^\/api\/posts\/([a-zA-Z0-9_-]+)\/comments$/); if (request.method.toUpperCase() === 'POST' && commentMatch) { const post: any = await env.DB.prepare('SELECT user_id FROM posts WHERE id = ?').bind(commentMatch[1]).first(); return { type: 'comment', postId: commentMatch[1], targetUserId: post?.user_id || null }; }
  return null;
}
async function persistActionNotification(context: any, response: Response, request: Request, env: Env, actorId: string | null): Promise<void> {
  if (!context || !actorId || !response.ok || context.wasExisting) return; if (context.type === 'like' || context.type === 'follow') { if (context.targetUserId) await createSocialNotification(env.DB, context.targetUserId, actorId, context.type, context.type === 'like' ? context.postId : undefined); return; }
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const url = new URL(routedRequest.url); let path = url.pathname; if (path.startsWith('/.netlify/functions/api')) path = path.replace('/.netlify/functions/api', '/api');
    ...
    if (env?.DB && typeof env.DB.prepare === 'function') {
      ...
      if (path === '/api/cpal_postback') { const rewritten = new URL(routedRequest.url); rewritten.pathname = '/api/earn/postback/cpalead'; routedRequest = new Request(rewritten.toString(), routedRequest); }
      if (isPr2Route) { const rewardsResponse = await handlePr2HardenedRequest(routedRequest, env, db); if (rewardsResponse) return rewardsResponse; }
    }
    const actorId = await authenticateSocialRequest(routedRequest, env.DB, secret).catch(() => null); const notificationContext = await captureNotificationContext(routedRequest, env, actorId); let response = await handleServerlessRequest(routedRequest, env); response = await normalizePostMedia(response, routedRequest); await persistActionNotification(notificationContext, response, routedRequest, env, actorId).catch((error) => { console.warn('[Sphere Notifications] Could not persist notification:', error); }); return response;
  },
};