...
function workerCorsHeaders(request: Request): Record<string, string> {
  ...
  const configuredOrigins = [envValueForCors(request, 'FRONTEND_ORIGIN'), envValueForCors(request, 'FRONTEND_NETLIFY_ORIGIN'), 'https://spheres.com.ng', 'https://sphereis.netlify.app'].filter(Boolean);
  const allowed = !origin || origin === 'null' || configuredOrigins.includes(origin) || /^http:\/\/(localhost|127\.0\.0\.1)(:\d+)?$/i.test(origin);
  return {
...
function envValueForCors(request: Request, key: string): string {
  // Worker CORS is intentionally pinned to the known production origins; request has no env access here.
  if (key === 'FRONTEND_ORIGIN') return 'https://spheres.com.ng';
  if (key === 'FRONTEND_NETLIFY_ORIGIN') return 'https://sphereis.netlify.app';
  return '';
}
...
async function guardLoginRequest(request: Request, env: Env): Promise<Response | null> {
  ...
  if (!row) return workerErrorResponse(request, 'Invalid username/email or password.', 401);
  return null;
}
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (env?.DB && typeof env.DB.prepare === 'function') {
      ...
      const cpaleadResponse = await handleCpaleadRequest(routedRequest, env, db); if (cpaleadResponse) return cpaleadResponse;
